...
int main(int argc, char* argv[]) {
    ...
    vector<string> block_header_ends_list = parse_header_list(header_list, "block header lengths");
    vector<string> block_ends_list = parse_header_list(header_list, "block lengths");
    vector<string> block_sizes_list = parse_header_list(header_list, "block sizes");
    cout << "Done." << endl << endl;

    // 4. opening index file
    ...
    vector<string> query_list = split_string(config_options["query_coordinates"], ',');
    ...
    for (int q_idx = 0; q_idx < query_list.size(); q_idx++){
        ...
        if (block_idx == 0){
            block_header_length = stoi(block_header_ends_list[block_idx]);
            block_length = stoi(block_ends_list[block_idx]) - block_header_length;
        }
        else {
            block_header_length = stoi(block_header_ends_list[block_idx]) - stoi(block_ends_list[block_idx-1]);
            block_length = stoi(block_ends_list[block_idx]) - stoi(block_ends_list[block_idx-1]) - block_header_length;
        }
